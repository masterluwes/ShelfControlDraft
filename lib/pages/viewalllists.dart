...
class _ViewAllListsPageState extends State<Viewalllist> {
                ...
                ElevatedButton(
                  ...
                  onPressed: enabled
                      ? () async {
                          ...
                          final deleted = await Navigator.of(parentContext)
                              .push<bool>(
                                MaterialPageRoute(
                                  builder: (_) =>
                                      ListItemsPage(listTitle: name),
                                ),
                              );

                          if (deleted == true && mounted) {
                            setState(() => _lists.remove(newMeta));
                          }
                        }
                      : null,
  ...
  // Align these category strings to your app’s canonical categories.
  ...
  List<GenItem> _generateItemsForMode(GenMode mode, {double budget = 0}) {
    // You can refine logic per mode/budget. This keeps items labeled with the correct category.
    final base = <GenItem>[
      GenItem(
        name: 'Orange Juice',
        brand: 'Minute Maid',
