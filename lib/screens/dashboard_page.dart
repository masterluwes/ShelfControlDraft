...
class HouseholdDropdown extends StatefulWidget {
  ...
  @override
  State<HouseholdDropdown> createState() => _HouseholdDropdownState();
}
...
class _HouseholdDropdownState extends State<HouseholdDropdown> {
  ...
  void initState() {
    super.initState();
    // Ensure a household is selected when the widget is first built.
    // This is a safe place for a one-time side-effect.
    if (widget.firestoreService.selectedHouseholdId == null ||
        !widget.households.any((h) => h.id == widget.firestoreService.selectedHouseholdId)) {
      if (widget.households.isNotEmpty) {
        final personalHousehold =
            widget.households.firstWhereOrNull((h) => h.isPersonal);
        // Use a post-frame callback to avoid calling setState during a build.
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            widget.firestoreService.selectedHouseholdId =
                personalHousehold?.id ?? widget.households.first.id;
          }
        });
      }
    }
  ...
  @override
  Widget build(BuildContext context) {
    // Sort households to put personal household first for display
    final sortedHouseholds = List<Household>.from(widget.households)..sort((a, b) {
    ...
    return Container(
      ...
      child: DropdownButton<String>(
        value: widget.firestoreService.selectedHouseholdId,
        items: sortedHouseholds
            .map(
              (h) => DropdownMenuItem(
                value: h.id,
                child: Text(
        ...
        onChanged: (value) {
          if (value != null) {
            widget.firestoreService.selectedHouseholdId = value;
          }
        },
