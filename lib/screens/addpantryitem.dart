...
class AddPantryItem extends StatefulWidget {
  ...
  const AddPantryItem({
    ...
    required this.onBack, required Null Function(dynamic updatedItem) onSave,
  });
...
class _AddPantryItemState extends State<AddPantryItem> {
  ...
  Future<void> _showCancelledDialog() async {
    await showDialog(
      ...
      builder: (BuildContext context) {
        // Automatically close the dialog after 3 seconds (same as success dialog)
        Future.delayed(const Duration(seconds: 3), () {
          if (mounted && Navigator.of(context).canPop()) {
            Navigator.of(context).pop();
          }
        ...
        return AlertDialog(
          ...
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
          content: Column(
            // Forces the dialog to wrap tightly around its content
            mainAxisSize: MainAxisSize.min, 
            crossAxisAlignment: CrossAxisAlignment.center,
            children: <Widget>[
              // TOP SPACING (Same as Success dialog)
              const SizedBox(height: 20),
              Text(
                'Cancelled!',
                textAlign: TextAlign.center,
                style: TextStyle(
                  // Uses red errorColor as requested
                  color: errorColor, 
                  fontWeight: FontWeight.bold,
                  fontSize: 24,
                ),
              ),
              // MIDDLE SPACING (Same as Success dialog)
              const SizedBox(height: 10),
              const Text(
                'Register pantry item cancelled!',
                textAlign: TextAlign.center,
                style: TextStyle(
                  color: Colors.black,
                  fontWeight: FontWeight.normal,
  ...
  // 🚀 MODIFIED: Function to display the confirmation dialog (image left)
  // Ensures exact visual copy of the provided image (layout, button colors/shapes)
              ...
              Row(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: TextButton(
                      style: TextButton.styleFrom(
                        // Exact green color from the image
                      ...
                      onPressed: () {
                        ...
                        _showCancelledDialog(); // Show the final cancelled dialog
                      },
                      child: const Text(
                        'Yes',
                        style: TextStyle(
                  ...
                  SizedBox(
                    ...
                    child: TextButton(
                      style: TextButton.styleFrom(
                        // Exact grey color from the image
                      ...
                      child: const Text(
                        'No',
                        style: TextStyle(
                          color: Colors.white, // White text for 'No' to match the image
                          fontWeight: FontWeight.bold,
                          fontSize: 16,
                        ),
                      ),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          // 🚀 MODIFIED: Call the cancel confirmation dialog instead of navigating back directly
